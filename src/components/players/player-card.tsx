...
export function PlayerCard({ player, stats }: { player: Player; stats: ScorerRow }) {
  return (
    <article className="group pfc-player-ground pfc-card-shadow relative block aspect-3/4 overflow-hidden rounded-card transition-transform duration-200 hover:-translate-y-1 hover:pfc-card-shadow-hover">
      ...
      <div className="absolute inset-0 grid place-items-center overflow-hidden">
        ...
        {player.photoUrl ? (
          <Image
            ...
            alt={player.name}
            fill
            sizes="(max-width: 520px) 50vw, (max-width: 940px) 33vw, 240px"
      ...
      {/* name plate */}
      <div className="pfc-player-scrim absolute inset-x-0 bottom-0 flex items-end gap-2.5 px-3.5 pt-9 pb-3.5 sm:gap-3 sm:px-[18px] sm:pt-11 sm:pb-[18px]">
        <span className="shrink-0 font-display text-[30px] leading-[0.84] font-black tracking-[-0.05em] tabular-nums text-lime sm:text-[40px]">
          {player.playerNumber}
        </span>
        <div>
          ...
          {player.role ? (
            <p className="mt-[3px] text-xs text-white/55">{player.role}</p>
          ) : null}
        </div>
      </div>
    </article>
